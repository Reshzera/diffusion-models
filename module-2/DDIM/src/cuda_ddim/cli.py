...
from cuda_ddim.data import download_cifar10, download_oxford_flowers, image_batches, list_images
from cuda_ddim.diffusion import DDIMSchedule
from cuda_ddim.model import DDIMUNet
...
def build_model(args, device: torch.device) -> DDIMUNet:
    return DDIMUNet(
...
def train(args) -> None:
    ...
    schedule = DDIMSchedule(timesteps=args.timesteps, schedule=args.noise_schedule, device=device)
    ...
    for step in progress:
        ...
        if args.sample_every and step % args.sample_every == 0:
            samples = schedule.sample(
                model,
                (args.sample_count, args.channels, args.image_size, args.image_size),
        ...
        if args.save_every and step % args.save_every == 0:
            checkpoint_path = checkpoint_dir / f"ddim_step_{step:06d}.pt"
            torch.save(model.state_dict(), checkpoint_path)
            progress.write(f"Saved checkpoint: {checkpoint_path}")

    final_path = output_dir / "ddim.pt"
...
def sample(args) -> None:
    ...
    images = schedule.sample(
        model,
        (args.count, args.channels, args.image_size, args.image_size),
        sampler=args.sampler,
        sample_steps=args.sample_steps,
        eta=args.ddim_eta,
    )
...
def smoke(args) -> None:
    ...
    samples = schedule.sample(
        model,
        (2, args.channels, args.image_size, args.image_size),
        sampler=args.sampler,
        sample_steps=args.sample_steps,
        eta=args.ddim_eta,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train and sample a small CUDA/PyTorch DDIM study model.")
    ...
    train_parser.add_argument("--output-dir", default="runs/cuda-ddim")
    ...
    sample_parser.add_argument("--output", default="runs/cuda-ddim/samples.png")
