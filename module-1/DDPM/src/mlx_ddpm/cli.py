...
from mlx_ddpm.data import download_cifar10, download_oxford_flowers, image_batches, list_images
from mlx_ddpm.diffusion import DDPMSchedule
from mlx_ddpm.model import DDPMUNet
...
def train(args) -> None:
    ...
    schedule = DDPMSchedule(timesteps=args.timesteps)
    ...
    for step in progress:
        ...
        if args.sample_every and step % args.sample_every == 0:
            samples = schedule.sample(model, (args.sample_count, args.image_size, args.image_size, args.channels))
        ...
        if args.save_every and step % args.save_every == 0:
            checkpoint_path = checkpoint_dir / f"ddpm_step_{step:06d}.safetensors"
            model.save_weights(str(checkpoint_path))
            progress.write(f"Saved checkpoint: {checkpoint_path}")

    final_path = output_dir / "ddpm.safetensors"
...
def sample(args) -> None:
    ...
    schedule = DDPMSchedule(timesteps=args.timesteps)
    images = schedule.sample(model, (args.count, args.image_size, args.image_size, args.channels))
...
def smoke(args) -> None:
    ...
    samples = schedule.sample(model, (2, args.image_size, args.image_size, args.channels))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train and sample a small MLX DDPM.")
    ...
    train_parser.add_argument("--output-dir", default="runs/ddpm")
    ...
    sample_parser.add_argument("--output", default="runs/ddpm/samples.png")
